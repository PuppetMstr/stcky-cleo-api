...
//
// Side effects:
//   1. Inserts a new doc into the `users` collection with email/name/token
//   2. Ingests every pre-auth turn into the substrate under the new token,
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // ingest pre-auth turns under the new user's token (best-effort)
    ...
    for (const t of turns) {
      ...
      const speaker = (t.role === 'sticky') ? 'STCKY' : name;
      try {
